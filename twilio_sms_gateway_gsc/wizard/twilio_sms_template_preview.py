...
class TwilioSMSTemplatePreview(models.TransientModel):
    _name = "twilio.sms.template.preview"
    _description = "Twilio SMS Template Preview"
    ...
    def default_get(self, fields):
        result = super(TwilioSMSTemplatePreview, self).default_get(fields)
        sms_template_id = self.env.context.get('default_sms_template_id')
        if not sms_template_id or 'resource_ref' not in fields:
            return result
        sms_template = self.env['twilio.sms.template'].browse(sms_template_id)
        res = self.env[sms_template.model_id.model].search([], limit=1)
        if res:
            result['resource_ref'] = '%s,%s' % (sms_template.model_id.model, res.id)
        return result
    
    sms_template_id = fields.Many2one('twilio.sms.template', required=True, domain="[('model_id', '=', False)]", ondelete='cascade')
    ...
    def _compute_sms_template_fields(self):
        for wizard in self:
            if wizard.sms_template_id and wizard.resource_ref:
                wizard.message = wizard.sms_template_id._render_field('message', [wizard.resource_ref.id], set_lang=wizard.lang)[wizard.resource_ref.id]
            else:
